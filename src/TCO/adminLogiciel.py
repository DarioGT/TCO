...
class CoutAnnuelInline(globale.admin.TabularInline):
    model = CoutAnnuel
    fk_name = 'logiciel'
    extra = 1


class CoutAdheranceInline(globale.admin.TabularInline):
    model = CoutAdherance
    fk_name = 'logiciel'
    extra = 1
...
class DiscussionLogicielInline(globale.admin.TabularInline):
    model = DiscussionLogiciel
    fk_name = 'logiciel'
    extra = 1


class SourceslInline(globale.admin.TabularInline):
#class SourceslInline(globale.admin.StackedInline):
    model = Sources
    fk_name = 'logiciel'
    extra = 1
...
class LogicielAdmin(globale.admin.ModelAdmin):
